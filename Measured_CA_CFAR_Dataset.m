...
[FileName,radarDatasetPath,filter_index]=uigetfile('D:\EEE4022S\OneDrive_2022-09-27\07 Radar datasets');
...
detect_Column = zeros(count,size(rangeProfiles,2));

%detectionsCombined_plot(detect_Column, count, start_td, hop_length_td,start_bin,c,Bandwidth_Hz,PRF_Hz);

% take a small subset of the range profiles 
subset = rangeProfiles(start:winLen+start-1,:);

%apply hamming window function to suppress the sidelobes 
hammerWindow = repmat(hamming(winLen),1,size(subset,2));		

window_set = subset.*hammerWindow;						
